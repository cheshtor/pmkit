...
(
    ...
    `type`          TINYINT(2) COMMENT '需求类型。1 - 业务需求; 2 - 拓展需求；3 - 故障修复',
    `demander`      VARCHAR(64) COMMENT '需求方',
    `priority`      TINYINT(2) COMMENT '优先级。1 - 重要且紧急；2 - 重要不紧急；3 - 紧急不重要；4 - 不紧急不重要',
    `influence`     TINYINT(2) COMMENT '影响程度。1 - 高；2 - 中；3 - 低；4 - 无',
    `owner`         BIGINT(20) DEFAULT 0 COMMENT '需求所有人，即认领此需求的人，负责排期',
    `tracer`        BIGINT(20) DEFAULT 0 COMMENT '需求追踪人，默认为创建人，负责验收需求',
    `status`        TINYINT(2) COMMENT '需求状态。1 - 待认领; 2 - 待审核；3 - 待排期；4 - 待实施；5 - 实施中；6 - 待交付；7 - 验收中；8 - 已交付',
    `delete`        TINYINT(1) DEFAULT 0 COMMENT '删除。0 - 可用; 1 - 删除',
    `create_by`     BIGINT(20) DEFAULT 0 COMMENT '创建人 ID',
    `create_time`   BIGINT(20) DEFAULT 0 COMMENT '创建时间',
    `modified_by`   BIGINT(20) DEFAULT 0 COMMENT '修改人 ID',
